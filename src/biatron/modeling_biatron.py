...
class BiatronRotaryEmbedding(nn.Module):
    inv_freq: torch.Tensor  # fix linting for `register_buffer`

    def __init__(self, config: BiatronConfig, device=None):
        super().__init__()
...
class BiatronMLP(nn.Module):
    def __init__(self, config):
        ...
        self.act_fn = F.gelu # Only GELU is supported for now
...
def eager_attention_forward(
    ...
    **kwargs: Unpack[TransformersKwargs],
    ...
    attn_weights = torch.matmul(query, key_states.transpose(2, 3)) * scaling # ok
...
class BiatronAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""

    def __init__(self, config: BiatronConfig, layer_idx: int):
        super().__init__()
        self.config = config
...
class BiatronDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: BiatronConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = BiatronAttention(config=config, layer_idx=layer_idx)

        self.mlp = BiatronMLP(config)
        self.input_layernorm = nn.LayerNorm(config.hidden_size, eps=config.norm_eps, bias=True, elementwise_affine=True)
        self.post_attention_layernorm = nn.LayerNorm(config.hidden_size, eps=config.norm_eps, bias=True, elementwise_affine=True)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        #print("befor attention", hidden_states)
...
class BiatronPreTrainedModel(PreTrainedModel):
    config: BiatronConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["BiatronDecoderLayer"]
    ...
    _can_record_outputs = {
        "hidden_states": BiatronDecoderLayer,
        "attentions": BiatronAttention,
    }


@auto_docstring
class BiatronModel(BiatronPreTrainedModel):
    def __init__(self, config: BiatronConfig):
        ...
        self.layers = nn.ModuleList(
            [BiatronDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.norm = nn.LayerNorm(config.hidden_size, eps=config.norm_eps, bias=True, elementwise_affine=True)
        self.rotary_emb = BiatronRotaryEmbedding(config=config)
        self.gradient_checkpointing = False

        # Initialize weights and apply final processing
        self.post_init()

    #@check_model_inputs #issue with new transformers version
    @auto_docstring
    def forward(
        self,
...
class BiatronForCausalLM(BiatronPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = BiatronModel(config)
        self.vocab_size = config.vocab_size
